...
describe('TcLayerImpl.cleanupOrphanedTabs', () => {
	let browserApi: BrowserApi
	...
	it('removes TC-created blank tabs not in preTabIds', async () => {
		const extensionId = '{c607c8df-14a7-4f28-894f-29e8722976af}'
		;(browserApi.management.get as ReturnType<typeof vi.fn>).mockResolvedValue({
			id: extensionId,
			name: 'Temporary Containers',
		...
		const orphanTab = { id: 200, index: 1, url: 'about:blank', cookieStoreId: 'firefox-tmp-99', windowId: 1 }
		;(browserApi.tabs.query as ReturnType<typeof vi.fn>).mockResolvedValue([orphanTab])
		;(browserApi.runtime.sendMessage as ReturnType<typeof vi.fn>).mockImplementation(
			async (_extId: string, msg: Record<string, unknown>) => {
				if (msg['method'] === 'isTempContainer') return true
				return false
			}
		)
		...
		const preTabIds = new Set([1, 2, 3])
		const cleanup = layer.cleanupOrphanedTabs(1, preTabIds)

		// Advance through all 6 iterations (each 150ms)
		for (let i = 0; i < 6; i++) {
			await vi.advanceTimersByTimeAsync(150)
		}
		await cleanup

		expect(browserApi.tabs.remove).toHaveBeenCalledWith(200)
	})

	it('does not remove pre-existing tabs', async () => {
		const extensionId = '{c607c8df-14a7-4f28-894f-29e8722976af}'
		;(browserApi.management.get as ReturnType<typeof vi.fn>).mockResolvedValue({
			id: extensionId,
			name: 'Temporary Containers',
		...
		// tab id 200 is in preTabIds, so should NOT be removed
		const existingTab = { id: 200, index: 1, url: 'about:blank', cookieStoreId: 'firefox-tmp-99', windowId: 1 }
		;(browserApi.tabs.query as ReturnType<typeof vi.fn>).mockResolvedValue([existingTab])
		;(browserApi.runtime.sendMessage as ReturnType<typeof vi.fn>).mockResolvedValue(true)

		const layer = new TcLayerImpl({ browserApi, logger: console })
		await layer.initialize()

		const preTabIds = new Set([200, 1, 2])
		const cleanup = layer.cleanupOrphanedTabs(1, preTabIds)
	...
	it('polls up to 6 times', async () => {
		...
		const cleanup = layer.cleanupOrphanedTabs(1, new Set([1, 2]))
		...
		expect(browserApi.tabs.query).toHaveBeenCalledTimes(6)
	})
})
